...
#  convolution.py
#-------------------
# Ce programme implante un calcul de convolution sur une image en utilisant les processu
...
if __name__=="__main__":
    ...
    print(taille_texte)
    print(texte[taille_texte-1])
    # Avec le multi-processus, il n'y a pas de mémoire commune implicite
    # Il faut donc explictement se décalrer de la mémoire partagée
    # Déclaration d'yune zone de mémoire partagée pour le fichier de sortie
    shm = shared_memory.SharedMemory(create=True, size=taille_texte)
    type(shm.buf)
    sortie = shm.buf
    ...
    for n in range(0, nbFils):
        if n < nbFils-1 : p = Process(target=convolution, args=(n, temp, filtre, taille_filtre, texte, th_image, tv_image, sortie))
        else : p = Process(target=convolution, args=(n, temp2, filtre, taille_filtre, texte, th_image, tv_image, sortie))
        processus.append(p)
        p.start()
    # Attente de la fin d'exécution des processus
    for p in processus:
        p.join()
    #===============================================
