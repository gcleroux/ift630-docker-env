...
void Program::init(Contexte enviro, std::string nomFichier, std::string nomKernel)
{
    ...
    // Chargement du code source du noyau */
    ...
    // Association du code source au contexte
...
void Program::execute(int dim, int nbPcs, int nbGroupe, T resul)
{
   if (dim == 1)
   { 
      if (nbGroupe == 0) err = listeCmd.enqueueNDRangeKernel(noyau, cl::NullRange, cl::NDRange(nbPcs), cl::NullRange);
      else err = listeCmd.enqueueNDRangeKernel(noyau, cl::NullRange, cl::NDRange(nbPcs), cl::NDRange(nbPcs/nbGroupe)); 
      std::cout<< "Execution : " << err<< std::endl;
      listeCmd.finish(); std::cout << "Exécution terminée........" << std::endl;
